...
let db;
massive(process.env.HEROKU_POSTGRESQL_MAUVE_URL)
  .then(dbInstance => {
...
app.get("/api/weather/:city/:state", (req, res, next) => {
  axios
    .get(
      `http://api.wunderground.com/api/7bd6e5a7f2af93d2/forecast10day/q/${req
        .params.state}/${req.params.city}.json`
    )
...
app.get("/api/tacos/:term", (req, res, next) => {
  axios
    .get("https://developer.nps.gov/api/v0/parks?limit=519&fields=addresses", {
      headers: {
...
passport.use(
  new GoogleStrategy(
    {
      clientID:
        "79316663432-tp4mvccldneduge8n1iell2n0bfgtc6p.apps.googleusercontent.com",
...
app.get("/api/s3", function(req, res, next) {
  ...
  s3.getSignedUrl("putObject", s3Config, function(err, response) {
    ...
    const data = {
      ...
      url: `https://${process.env.BUCKET_NAME}.s3.amazonaws.com/${req.query
        .file_name}`
    };
